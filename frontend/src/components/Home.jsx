...
function Home() {
  ...
  useEffect(() => {
    const fetchtodos = async () => {
      try {
        setLoading(true);
        const response = await axios.get("http://localhost:4003/todo/fetch", {
  ...
  const todoCreate = async () => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:4003/todo/create",
        {
          text: newTodo,
          completed: false,
        },
  ...
  const todoStatus = async (id) => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:4003/todo/update/${id}`,
        {
          ...todo,
  ...
  const todoDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:4003/todo/delete/${id}`, {
        withCredentials: true,
      });
  ...
  const logout = async () => {
    try {
      await axios.get("http://localhost:4003/user/logout", {
        withCredentials: true,
      });
